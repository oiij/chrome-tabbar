...
// node_modules/.pnpm/@oiij+utils@0.0.1/node_modules/@oiij/utils/dist/chunk-U4IIBN6E.js
var isFunction2 = (val) => typeof val === "function";
var isNode = typeof process !== "undefined" && process.versions != null && process.versions.node != null;
var isBrowser = typeof window === "object";
var isDarkMode = isBrowser ? window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches : false;

// node_modules/.pnpm/@oiij+use@0.0.1_typescript@5.7.3/node_modules/@oiij/use/dist/index.js
function useAutoRatio(ratio = 1) {
  const domRef = ref();
  const { width: targetWidth, height: targetHeight } = (domRef == null ? void 0 : domRef.value) ? useElementSize(domRef) : useWindowSize();
...
function useEventBusKey(key) {
  return Symbol(key);
}
function randomNum(min2, max2) {
  const num = Math.floor(Math.random() * (max2 - min2) + min2);
  return num;
}
function randomColor(min2, max2) {
  const r = randomNum(min2, max2);
  const g = randomNum(min2, max2);
  const b = randomNum(min2, max2);
  return `rgb(${r},${g},${b})`;
}
function useImageVerify(options) {
  ...
  function draw(type2 = "character") {
    var _a;
    let imgCode2 = "";
    const ctx = (_a = domRef.value) == null ? void 0 : _a.getContext("2d");
    if (!ctx)
      return imgCode2;
    ...
    if (type2 === "operation") {
      ...
      switch (tempArith) {
        case 1:
          imgCode2 = add_default(num1, num2).toString();
          codeShow = `${num1} + ${num2} = ?`;
          break;
        case 2:
          if (num1 < num2) {
          ...
          imgCode2 = subtract_default(num1, num2).toString();
          codeShow = `${num1} - ${num2} = ?`;
          break;
        default:
          imgCode2 = multiply_default(num1, num2).toString();
          codeShow = `${num1} × ${num2} = ?`;
          break;
      }
      for (let i = 0; i < codeShow.length; i += 1) {
        ctx.fillStyle = randomColor(50, 160);
        ctx.font = `${random_default(height * 2 / 4, height * 3 / 4)}px SimHei`;
        ctx.textBaseline = "top";
        const x = i * ((width - 10) / codeShow.length);
        const y = random_default(5, height / 4);
  ...
  onMounted(() => {
    ...
    drawImgCode();
    (_a = domRef.value) == null ? void 0 : _a.addEventListener("click", onClick);
  });
  onUnmounted(() => {
    var _a;
    (_a = domRef.value) == null ? void 0 : _a.removeEventListener("click", onClick);
  });
  return {
    domRef,
    code,
...
function useNumberAnimation(options) {
  const { from = 0, to, duration = 5 * 60 * 60, onProgress = void 0, fixed = 2 } = typeof options === "number" ? { to: options } : options;
  ...
  function run() {
    ...
    if (t >= duration) {
      value.value = to;
      onProgress == null ? void 0 : onProgress(to);
      return;
    }
    value.value = from + t * speed;
    onProgress == null ? void 0 : onProgress(value.value);
...
function useScanCode() {
  ...
  function onKeyDown(ev) {
    ...
    if (key === "Enter") {
      ...
      if (tempStr.length > 0) {
        code.value = tempStr;
        if (onScanFun && isFunction2(onScanFun))
          onScanFun(tempStr);
      }
    }
    if (!pending.value) {
...
async function useViewTransition(toggle, options) {
  ...
  const { x = 0, y = 0, effect = true, reverse: reverse2 = ref(true), reverseSelector = ".dark" } = options ?? {};
  ...
  return new Promise((resolve) => {
    const isReverse = toValue(reverse2);
...
function useWebSockets(url, options) {
  ...
  function destroy() {
    var _a, _b, _c, _d;
    close();
    (_a = socket.value) == null ? void 0 : _a.removeEventListener("open", onOpen);
    (_b = socket.value) == null ? void 0 : _b.removeEventListener("message", onMessage);
    (_c = socket.value) == null ? void 0 : _c.removeEventListener("close", onClose);
    (_d = socket.value) == null ? void 0 : _d.removeEventListener("error", onError);
    socket.value = void 0;
    onOpenFn = null;
    onMessageFn = null;
    onCloseFn = null;
...
lodash-es/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=@oiij_use.js.map
